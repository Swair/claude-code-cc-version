...
inline ThreadPool::ThreadPool(size_t num_threads, size_t max_pending)
    ...
    if (thread_count == 0) {
        thread_count = 1;  // 兜底：hardware_concurrency() 返回 0 时至少一个 worker
    }
...
inline void ThreadPool::WorkerLoop() {
    while (true) {
...

        {
            ...
            // 检查并发限制（max_concurrency_ == 0 表示无限制，需显式短路：
            // size_t 无符号，否则 active_tasks_ >= 0 恒真导致池子卡死）
            if (max_concurrency_.load() > 0 && active_tasks_.load() >= max_concurrency_.load()) {
                lock.unlock();
